...
CREATE TABLE ingredient (
  ...
  ingredient    VARCHAR(127) UNIQUE,
  calories      FLOAT,
  cost_per_unit NUMERIC(7, 2),
  is_kosher     BOOLEAN
);
CREATE TABLE recipe (
  recipe       VARCHAR(63) PRIMARY KEY,
  instructions TEXT,
  num_servings INT,
  course       VARCHAR(15),
  season       VARCHAR(15),
...
CREATE TABLE inventory (
  ing_id  CHAR(8) REFERENCES ingredient ON UPDATE CASCADE ON DELETE SET NULL,
  amount      NUMERIC(14, 6)
);
CREATE TABLE orders (
  ...
  filled        TIMESTAMP NULL
);

GRANT ALL PRIVILEGES ON all tables in schema testing TO restaurant;
GRANT CREATE ON SCHEMA testing TO restaurant;
...
INSERT INTO recipe VALUES ('Chicken Sandwich', 'Spread mayonnaise on a slice of bread. Place chicken on top, add a second slice of bread on top of that.', 1, 'Entree', 'Spring', FALSE);
INSERT INTO recipe VALUES ('Ham Sandwich', 'Spread mustard on two slices of bread. Place ham and cheese between them.', 1, 'Entree', 'Fall', FALSE);
INSERT INTO recipe VALUES ('French Fries', 'Slice potatoes and fry them.', 3, 'Side', 'All', FALSE);
INSERT INTO recipe VALUES ('Garden Salad', 'Assemble ingredients in a bowl and toss them together.', 8, 'Side', 'All', FALSE);
INSERT INTO recipe VALUES ('Hamburger', 'Form ground beef into patties. Cook patties to order. Place patties on bun. Serve open face', 1, 'Entree', 'All', FALSE);
INSERT INTO recipe VALUES ('Pumpkin Pie', 'Pour pie filling into pie crust. Bake at an appropriate temperature until done.', 8, 'Dessert', 'Fall', FALSE);
INSERT INTO recipe VALUES ('Ice Cream Sundae', 'Scoop ice cream into dish, and add chocolate sauce.', 1, 'Dessert', 'Spring', FALSE);
INSERT INTO recipe VALUES ('Milkshake', 'Add ice cream and milk to blender, and blend until smooth. Serve in a tall glass.', 2, 'Dessert', 'All', TRUE);
INSERT INTO recipe VALUES ('Ham and Mashed Potatoes', 'Roast ham in oven until hot. Boil potatoes until done, then mash and add milk to make them smooth.', 4, 'Entree', 'Spring', FALSE);
...
-- Start with enough inventory to make 10 of everything
INSERT INTO inventory
SELECT ingredient, sum(amount) * 10
